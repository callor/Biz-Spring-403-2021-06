...
public class CompDaoImplV1 implements CompDao {
	...
	public int insert(CompVO vo) {
		...
		Object[] params = new Object[] { vo.getCp_code(), vo.getCp_title(), vo.getCp_ceo(), vo.getCp_addr(),
				vo.getCp_tel() };
	...
	/*
	 * jdbcTemplate을 사용하여 QUERY를 실행할때 각 method에서 매개변술 받아 QUERY에 전달할텐데 이때 매개변수는
	 * primitive로 받으면 값을 변환시키는 어려움이 있다 권장사항으로 매개변수는 wrapper class type으로 설정 즉 숫자형일
	 * 경우 int, long 대신 Integer, Long 형으로 선언
	 * 
	 * vo에 담겨서 전달된 값은 Object[] 배열로 변환한 후 jdbcTemplate에 전달해 주어야 한다
	 * 
	 * 하지만, 1 ~ 2개 정도의 값을 전달할때 Object[] 배열로 변환 후 전달을 하면 Object 객체 저장소가 생성되고 메모리를
	 * 사용한다 이때 전달되는 변수가 wrapper class type 이면 Object[] 배열로 만들지 않고 바로 주입할수 있다
	 */
	@Override
	...
	/*
	 * tbl_company table에서 cpcode(출판사코드) 중 가장 큰값을 추출하기
	 */
	@Override
